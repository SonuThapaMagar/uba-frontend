    ...
    return (
      <DashboardLayout>
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded relative" role="alert">
          ...
          <span className="block sm:inline">{userError?.message || usersError?.message}</span>
        </div>
      </DashboardLayout>
    );
  }
      ...
      <div className="bg-white rounded-lg shadow">
        <div className="px-6 py-4 border-b flex justify-between items-center">
          ...
          {(isSuperAdmin || isAdmin) && (
            <Link to="/users" className="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700">
              View All Users
            </Link>
          )}
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {previewUsers.length === 0 ? (
                <tr>
                  <td colSpan={isSuperAdmin || isAdmin ? 5 : 4} className="text-center py-4 text-gray-500">
                    No users found.
                  </td>
                </tr>
              ) : (
                    ...
                    {(isSuperAdmin || isAdmin) && (
                      <td className="px-6 py-4 whitespace-nowrap space-x-2">
                        ...
                        {(isSuperAdmin || (isAdmin && user.role === 'USER')) && (
                          <Link to={`/users/editUser/${user.id}`} className="text-indigo-600 hover:text-indigo-900 ml-4">
                            Edit
                          </Link>
                        )}
                        {isSuperAdmin && user.role !== 'SUPER_ADMIN' && (
                          <Link to={`/users/deleteUser/${user.id}`} className="text-red-600 hover:text-red-900 ml-4">
                            Delete
                          </Link>
                        )}
                      </td>
